...
|---|---|---:|---:|---:|---:|
| Сингапур (DC1) | Индонезия, Филиппины | ~244 млн | **~12%** | **244 015** | **2 256** |
| Бангкок (DC2) | Таиланд, Вьетнам + ЮВА прочие | ~186 млн | **~10%** | **203 346** | **1 880** |
...
Расчёт для DC1 (Сингапур) — наиболее нагруженный ДЦ, 12% глобального трафика.
Для остальных ДЦ нагрузка пропорционально меньше.

| Параметр | Значение | Источник |
|---|---|---|
| Пиковый RPS на DC1 | **~244k RPS** | 12% × 2 033 458 |
| Пиковый трафик через L7 | **~18 Гбит/с** | 244k RPS × 10 КБ (средний размер ответа) |
| HTTP Throughput NGINX (16 CPU, 10GbE) | **8.8 Гбит/с** | [9] |
| SSL CPS NGINX (RSA 2048) | **56 715 CPS** | [10] |

**Ограничитель 1 — SSL Termination:**

Даже в худшем случае — 1 запрос на соединение, то есть keep-alive не работает совсем:

`244 000 CPS / 56 715 CPS = 4.3 → 5 серверов`

В реальности при работающем keep-alive CPS значительно меньше RPS,
поэтому SSL **не является определяющим ограничителем**.

**Ограничитель 2 — пропускная способность:**

`18 Гбит/с / 8.8 Гбит/с = 2.05 → 3 сервера`

**Проверка N+1** — при падении одного оставшиеся должны справиться с пиковой нагрузкой

Увеличиваем до 4 активных:

`3 × 8.8 = 26.4 Гбит/с > 18 Гбит/с` 

Определяющий ограничитель — **пропускная способность**.

Итого N+1: `4 + 1 =` **5 серверов NGINX на DC1**
